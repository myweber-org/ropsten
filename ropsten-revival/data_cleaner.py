...
def validate_dataset(df, required_columns=None, min_rows=1):
    ...
    if df.isnull().sum().sum() > len(df) * 0.5:
        return False, "More than 50% of data contains missing values"
...
if __name__ == "__main__":
    ...
    filtered = remove_outliers(df_with_outliers, 'values', method='iqr')
    print(f"\nOutlier removal: {len(df_with_outliers)} -> {len(filtered)} rows")
import numpy as np
import pandas as pd
