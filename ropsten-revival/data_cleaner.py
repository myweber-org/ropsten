...
def load_and_clean_csv(filepath, cleaning_strategy='mean'):
    ...
    try:
        ...
        print(f"Cleaned data: {len(df)} -> {len(df_clean)} rows")
...
if __name__ == "__main__":
    ...
    print(df_cleaned)import numpy as np
import pandas as pd
...
def process_dataset(file_path, column_name):
    """
    Load a dataset from file, remove outliers, and return cleaned data with statistics.
    
    Parameters:
    file_path (str): Path to the data file (CSV format)
    column_name (str): Column to process for outliers
