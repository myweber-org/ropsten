...
def clean_numeric_outliers(df: pd.DataFrame, column: str, 
    ...
    """
    Remove outliers from numeric column using percentile method.
...
def clean_dataframe(df: pd.DataFrame, 
    ...
    Args:
        df: Input DataFrame
        string_columns: Columns to normalize
        outlier_columns: Columns to clean outliers from
...
def clean_dataset(dataframe, numeric_columns, outlier_threshold=1.5, 
                  normalization_method='zscore', feature_range=(0, 1)):
    """
    ...
    Args:
        ...
        feature_range: output range for min-max normalization
    ...
    if normalization_method == 'zscore':
        ...
        cleaned_df = min_max_normalize(cleaned_df, numeric_columns, feature_range)
    else:
...
def validate_email_column(df, email_column):
    ...
    return df[email_column].str.match(email_pattern, na=False)import numpy as np
import pandas as pd
from scipy import stats
