...
if __name__ == "__main__":
    ...
    print(f"\nOriginal size: {len(sample_df)}")
    print(f"Cleaned size: {len(cleaned_df)}")
    print(f"Removed {len(sample_df) - len(cleaned_df)} outliers")
import pandas as pd
import numpy as np
...
def validate_data(df, required_columns=None, allow_nan=True, max_nan_ratio=0.1):
    ...
    if not allow_nan:
        ...
        if nan_columns:
            validation_results['is_valid'] = False
            validation_results['issues'].append(f"Columns with NaN values: {nan_columns}")
    else:
        for col in df.columns:
            ...
            if nan_ratio > max_nan_ratio:
                ...
                validation_results['issues'].append(
                    f"Column '{col}' has high NaN ratio: {nan_ratio:.2%}"
                )
...
if __name__ == "__main__":
    ...
    print(f"Cleaned shape: {cleaned_df.shape}")import numpy as np
import pandas as pd
...
class DataCleaner:
    ...
    def get_removal_stats(self):
        ...
        return {
            ...
            'rows_removed_percentage': (rows_removed / self.original_shape[0]) * 100 if self.original_shape[0] > 0 else 0,
            'original_columns': self.original_shape[1],
...
if __name__ == "__main__":
    ...
    removed_iqr = cleaner.remove_outliers_iqr()
    print(f"Removed {removed_iqr} rows")
