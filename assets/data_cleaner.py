...
def validate_data(data, required_columns=None, allow_nan=True, nan_threshold=0.5):
    ...
    if required_columns:
        ...
        if missing:
            ...
            validation_report['issues'].append(f"Missing required columns: {missing}")
    
    if not allow_nan:
        nan_columns = data.columns[data.isnull().any()].tolist()
        if nan_columns:
            validation_report['is_valid'] = False
            validation_report['issues'].append(f"Columns with NaN values: {nan_columns}")
    else:
        for column in data.columns:
            nan_ratio = data[column].isnull().sum() / len(data)
            if nan_ratio > nan_threshold:
                validation_report['high_nan_columns'].append(column)
                validation_report['issues'].append(f"Column '{column}' has {nan_ratio:.1%} NaN values")
