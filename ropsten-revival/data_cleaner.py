...
def clean_missing_values(df, strategy='mean'):
    ...
    if strategy == 'drop':
        ...
        cleaned_df = df.copy()
        for col in numeric_cols:
            cleaned_df[col].fillna(df[col].mode()[0], inplace=True)
    else:
...
if __name__ == "__main__":
    ...
    print("With threshold 2:", clean_data_with_threshold(sample_data, 2))import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    ...
    normalized = normalize_numeric_columns(cleaned, columns=['value'])
