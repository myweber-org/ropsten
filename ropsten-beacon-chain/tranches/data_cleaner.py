    ...
    if fill_missing and cleaned_df.isnull().sum().any():
        for column in cleaned_df.select_dtypes(include=[np.number]).columns:
            if cleaned_df[column].isnull().any():
                ...
                print(f"Filled {missing_count} missing values in column '{column}' with {missing_strategy}: {fill_value}")
...
def sample_data_cleaning():
    ...
    cleaned = clean_dataframe(df, missing_strategy='median')
    print("\nCleaned DataFrame:")
    print(cleaned)
...
if __name__ == "__main__":
    ...
    cleaner = DataCleaner(sample_df)
    cleaner.remove_outliers_iqr(['income', 'score']) \
           .fill_missing_mean(['age']) \
           .normalize_minmax(['age', 'income', 'score'])
...
if __name__ == "__main__":
    sample_data = {
        'values': [10, 12, 12, 13, 12, 11, 14, 13, 15, 102, 12, 14, 13, 12, 11, 14, 13, 12, 14, 15, 12, 13, 14, 12, 11]
    }
