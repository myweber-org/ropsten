...
if __name__ == "__main__":
    ...
    is_valid = validate_data(cleaned, ['A', 'B'])
...
def validate_data(data, required_columns, numeric_threshold=0.8):
    ...
    for column in required_columns:
        ...
        if missing_ratio > numeric_threshold:
            validation_results['high_missing'].append((column, missing_ratio))
        
        if data[column].nunique() == 1:
            validation_results['constant_columns'].append(column)
...
def handle_missing_values(data, strategy='mean', columns=None):
    ...
    if strategy == 'drop':
        ...
        for col in columns:
            if col in processed_data.columns and processed_data[col].isna().any():
                if strategy == 'mean':
