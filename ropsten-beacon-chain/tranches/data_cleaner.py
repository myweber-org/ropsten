...
if __name__ == "__main__":
    sample_data = {
        ...
        'Phone': ['1234567890', '(555) 123-4567', 'invalid', '9876543210'],
...
def process_data(file_path, output_path=None):
    ...
    try:
        ...
        if validate_dataframe(df):
            cleaned_df = clean_dataframe(df, fill_missing=True)
...
def clean_dataset(data, outlier_method='iqr', normalize_method='minmax', outlier_threshold=None):
    """
...
def validate_data(data, required_columns=None, allow_nan=False):
    ...
    if not isinstance(data, pd.DataFrame):
        ...
        validation_report['issues'].append('Input is not a pandas DataFrame')
    ...
    if not allow_nan:
        ...
        if not columns_with_nan.empty:
            validation_report['missing_values'] = columns_with_nan.to_dict()
            validation_report['issues'].append('Dataset contains NaN values')
    
    numeric_columns = data.select_dtypes(include=[np.number]).columns
    if len(numeric_columns) == 0:
        validation_report['issues'].append('No numeric columns found for processing')
        ...
        for col in numeric_cols:
            if df_clean[col].isnull().any():
                if fill_missing == 'mean':
                    ...
                    raise ValueError("Invalid fill_missing method. Use 'mean', 'median', 'mode', or 'drop'.")
