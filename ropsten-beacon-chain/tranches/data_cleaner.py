...
if __name__ == "__main__":
    ...
    if cleaned_df is not None:
        ...
        print(f"Data validation: {'Passed' if is_valid else 'Failed'}")import numpy as np
import pandas as pd
...
def validate_dataframe(df, required_columns):
    """
    Validate if dataframe contains required columns and has no null values.
    """
...
def clean_dataset(df, missing_strategy='mean', normalize=True, remove_outliers=False):
    """
    ...
    if normalize:
        df_clean = normalize_columns(df_clean, method='minmax')
