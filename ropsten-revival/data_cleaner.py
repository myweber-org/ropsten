...
def get_summary_statistics(data, numeric_columns=None):
    ...
    for column in numeric_columns:
        ...
        summary.loc[column, 'count'] = len(col_data)
        summary.loc[column, 'mean'] = col_data.mean()
        summary.loc[column, 'std'] = col_data.std()
        summary.loc[column, 'min'] = col_data.min()
        summary.loc[column, '25%'] = col_data.quantile(0.25)
        summary.loc[column, 'median'] = col_data.median()
        summary.loc[column, '75%'] = col_data.quantile(0.75)
        summary.loc[column, 'max'] = col_data.max()
        summary.loc[column, 'skew'] = col_data.skew()
        summary.loc[column, 'kurtosis'] = col_data.kurtosis()
