...
if __name__ == "__main__":
    ...
    cleaner.remove_outliers_zscore(['feature_a', 'feature_b'])
    ...
    for col in numeric_columns:
        if col in df.columns:
            validation_report[col] = {
                ...
                'cleaned_size': len(df),
                'removed_percentage': 100 * (1 - len(df) / len(original_df))
            }
...
def summary_statistics(dataframe, column):
    stats = {
...
if __name__ == "__main__":
    sample_data = {'values': [10, 12, 12, 13, 12, 11, 10, 100, 12, 14, 13, 12, 10, 9, 8, 15, 200]}
    df = pd.DataFrame(sample_data)
...
if __name__ == "__main__":
    ...
    is_valid, message = validate_data(cleaned_df, required_columns=['id', 'value'], min_rows=1)
    print(f"\nValidation: {is_valid} - {message}")
    
    outlier_free_df = remove_outliers_iqr(cleaned_df, columns=['value'])
