...
if __name__ == "__main__":
    sample_data = {
        'values': [10, 12, 12, 13, 12, 11, 14, 13, 15, 100, 12, 12, 13, 12, 11, 14, 13, 15, 12, 11]
    }
...
if __name__ == "__main__":
    sample_data = {
        ...
        'age': [25, 30, 35, 40, 45, 45, 1000],
        'score': [85.5, 92.0, None, 78.5, 88.0, 88.0, 95.5]
    }
    ...
    print(f"\nDataset after removing age outliers: {len(df_no_outliers)} rows")
import pandas as pd
import numpy as np
import re
...
def clean_dataset(df, drop_duplicates=True, fill_missing='mean'):
    ...
    print(f"Cleaning complete. Missing values: {final_missing}, Duplicates: {final_duplicates}")
...
def validate_dataframe(df, required_columns=None):
    ...
    for col in df.columns:
        ...
        if unique_types > 1:
            validation_results['issues'].append(f'Column {col} has mixed data types')
...
def clean_dataset(input_path, output_path, numeric_columns):
    ...
    print(f"After outlier removal shape: {df_no_outliers.shape}")
...
if __name__ == "__main__":
    ...
    cleaned_data = clean_dataset(input_file, output_file, numeric_cols)import pandas as pd
...
def clean_dataset(data, numeric_columns=None, outlier_multiplier=1.5):
    ...
    return cleaned_data.reset_index(drop=True)import pandas as pd
