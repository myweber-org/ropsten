...
def clean_dataset(df, outlier_columns=None, normalize_columns=None, fill_missing=True):
    cleaner = DataCleaner(df)
    
    if outlier_columns is not None:
        cleaner.remove_outliers_iqr(outlier_columns)
        
    if fill_missing:
        cleaner.fill_missing_median()
        
    if normalize_columns is not None:
        cleaner.normalize_minmax(normalize_columns)
