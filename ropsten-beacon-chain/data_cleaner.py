...
def validate_data(df, required_columns=None, min_rows=10):
    ...
    if any(df[numeric_cols].isin([np.inf, -np.inf]).any()):
...
if __name__ == "__main__":
    ...
    # Validate the data
    is_valid, message = validate_data(normalized, min_rows=3)
...
if __name__ == "__main__":
    ...
    print(f"Cleaned shape: {result.shape}")import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    ...
    final_df = remove_outliers_iqr(cleaned_df)
    print(final_df)
import pandas as pd
import numpy as np
