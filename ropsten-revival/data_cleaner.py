...
#     is_valid, message = validate_dataset(cleaned, required_columns=['A', 'B', 'C'], min_rows=1)
...
def validate_data(data, check_missing=True, check_inf=True):
    """
    ...
    if check_inf:
        inf_counts = data.applymap(lambda x: np.isinf(x) if isinstance(x, (int, float)) else False).sum()
