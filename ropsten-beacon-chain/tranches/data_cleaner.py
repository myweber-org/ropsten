...
def validate_data_types(data, expected_types):
    ...
    for column, expected_type in expected_types.items():
        ...
        if expected_type == 'numeric':
            is_valid = np.issubdtype(actual_type, np.number)
        elif expected_type == 'categorical':
            is_valid = data[column].dtype == 'object' or data[column].dtype.name == 'category'
        elif expected_type == 'datetime':
            is_valid = np.issubdtype(actual_type, np.datetime64) or pd.api.types.is_datetime64_any_dtype(data[column])
        else:
