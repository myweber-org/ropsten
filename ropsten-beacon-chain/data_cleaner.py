...
def validate_data(data, column, expected_range=None):
    if expected_range:
        min_val, max_val = expected_range
        invalid_data = data[(data[column] < min_val) | (data[column] > max_val)]
        if len(invalid_data) > 0:
            print(f"Warning: {len(invalid_data)} values outside expected range [{min_val}, {max_val}]")
            return False
    return True