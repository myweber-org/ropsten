...
def create_sample_data():
    ...
    df.loc[outlier_indices, 'income'] = df['income'].max() * 10
...
if __name__ == "__main__":
    ...
    filled = cleaner.fill_missing_with_strategy(strategy='median')
...
def clean_dataset(df, numeric_columns):
    ...
    for col in numeric_columns:
        if col in df.columns and pd.api.types.is_numeric_dtype(df[col]):
            original_stats[col] = calculate_basic_stats(cleaned_df, col)
            cleaned_df = remove_outliers_iqr(cleaned_df, col)
    
    final_stats = {}
    for col in numeric_columns:
        if col in cleaned_df.columns and pd.api.types.is_numeric_dtype(cleaned_df[col]):
            final_stats[col] = calculate_basic_stats(cleaned_df, col)
...
if __name__ == "__main__":
    ...
    cleaned_df, stats_comparison = clean_dataset(sample_df, ['A', 'B', 'C'])
    
    print("Cleaned dataset shape:", cleaned_df.shape)
    print("Rows removed:", len(sample_df) - len(cleaned_df))
    
    for col in ['A', 'B', 'C']:
        print(f"\nColumn {col}:")
        print(f"  Original mean: {stats_comparison['original'][col]['mean']:.2f}")
        print(f"  Cleaned mean: {stats_comparison['cleaned'][col]['mean']:.2f}")
import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    sample_data = {
        'values': [10, 12, 12, 13, 12, 11, 10, 100, 12, 14, 13, 12, 10, 9, 8, 150, 11, 12]
    }
    df = pd.DataFrame(sample_data)
