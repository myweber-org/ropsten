...
if __name__ == "__main__":
    ...
    is_valid, message = validate_dataframe(cleaned_df, ['feature1', 'feature2'])
    print(f"\nValidation: {message}")
import pandas as pd
import numpy as np
...
def clean_dataset(df, missing_strategy='mean', outlier_removal=True, standardize=True):
    """
    Comprehensive data cleaning pipeline.
    ...
    if missing_strategy != 'none':
        df_clean = clean_missing_values(df_clean, strategy=missing_strategy, columns=numeric_cols)
...
def clean_dataset(df, remove_duplicates=True, fill_method='drop'):
    ...
    fill_method (str): Method to handle missing values. 
                       Options: 'drop' (drop rows), 'fill_mean' (fill with column mean),
                       'fill_median' (fill with column median), 'fill_zero' (fill with 0).
