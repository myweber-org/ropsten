...
if __name__ == "__main__":
    ...
    print(cleaned[['feature_a_normalized', 'feature_a_standardized']].head())import numpy as np
import pandas as pd
...
def load_and_clean_csv(filepath: str, 
    ...
    if cleaning_steps is None:
        cleaning_steps = ['remove_duplicates', 'handle_missing_values']
...
if __name__ == "__main__":
    ...
    print(validate_dataset(cleaned_df))import pandas as pd
...
if __name__ == "__main__":
    ...
    cleaned = clean_dataset(df, fill_missing={'A': 0, 'B': 0, 'C': 'unknown'})
    print("\nCleaned DataFrame:")
    print(cleaned)
    
    is_valid = validate_data(cleaned, required_columns=['A', 'B'])
    print(f"\nData validation passed: {is_valid}")def remove_duplicates(input_list):
    seen = set()
    result = []
