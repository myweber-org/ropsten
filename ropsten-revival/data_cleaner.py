...
def process_dataframe(df, column):
    """
    Complete processing pipeline: remove outliers and return statistics.
...
def get_cleaning_summary(df, cleaned_df, numeric_columns=None):
    """
    ...
    summary = {
        ...
        'missing_values_before': df[numeric_columns].isna().sum().to_dict(),
        'missing_values_after': cleaned_df[numeric_columns].isna().sum().to_dict()
    }
...
def get_data_summary(df):
    """
    Generate a summary of the DataFrame including outlier information.
...
if __name__ == "__main__":
    ...
    print(cleaned.describe())import numpy as np
import pandas as pd
from scipy import stats
