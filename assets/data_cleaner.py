...
def validate_email_column(df, email_column):
    ...
    total_count = len(df)
    print(f"Found {valid_count} valid emails out of {total_count}.")
...
def create_sample_data():
    ...
    df.loc[missing_indices, 'feature_a'] = np.nan
...
class DataCleaner:
    ...
    def normalize_columns(self, columns: list, method: str = 'minmax') -> 'DataCleaner':
        for col in columns:
...
if __name__ == "__main__":
    ...
    stats_a = calculate_basic_stats(df, 'A')
    print("Statistics for column A:")
...
def create_sample_data():
    ...
    df = pd.DataFrame(data)
    df.iloc[np.random.choice(1000, 100), 0] = np.nan
    df.iloc[np.random.choice(1000, 50), 1] = np.nan
